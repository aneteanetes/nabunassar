...
namespace ioi.Widgets.UserInterfaces.GameWindows.Manipulations.Components
{
    internal class HorizontalIconPanel : HorizontalStackPanel
    {
        ...
        public HorizontalIconPanel(GameContentManager content, List<IconButton> iconButtons, Func<Image,Vector2> titlePosition=null, HorizontalAlignment btnAligment= HorizontalAlignment.Left)
        {
        ...
        private Image AddButton(HorizontalStackPanel panel, IconButton iconButton, Action click, string title, int paddingLeft = 0, int paddingRight = 0)
        {
            ...
            img.MouseEntered += (s, e) =>
            {
                ...
                GameHost.Game.AddDesktopWidget(new TitleWidget(GameHost.Game, title, titlePos, Color.White));
            };
            img.MouseLeft += (s, e) =>
            {
                if (iconButton.IsReactOnClick)
                ...
                GameHost.Game.RemoveDesktopWidgets<TitleWidget>(_selectedImage != img ? 1 : 0);
            };
            img.TouchDown += (s, e) =>
            {
                if (iconButton.IsReactOnClick)
