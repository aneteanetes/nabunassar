...
namespace ioi.Widgets.UserInterfaces
{
    internal class ChatWindow : ScreenWidgetWindow
    {
        ...
        public ChatWindow(GameHost game) : base(game)
        {
            ...
            //if (scaleVector.X != 1 && scaleVector.X != 0)
            //{
        ...
        {
            GameHost.Game.GameState.Cursor.SetCursor();
        }

        private static void Label_MouseEntered(object sender, MyraEventArgs e)
        {
            GameHost.Game.GameState.Cursor.SetCursor("info");
        }
