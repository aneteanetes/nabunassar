...
namespace ioi.Widgets.UserEffects
{
    internal class RevealCircle : ScreenWidget
    {
        ...
        public RevealCircle(GameHost game, RevealAbility ability) : base(game)
        {
        ...
        private Profile _radiation = Profile.Circle(1, CircleRadiation.Out);
        ...
        public override void LoadContent()
        {
            ...
            partyOrigin = Game.CameraMain.WorldToScreen(partyOrigin);
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            _revealArea.Position = Game.CameraMain.ScreenToWorld(_mousepos);

            if (!_isDissapearing)
            {
                _particleEffect.Position = Game.CameraMain.ScreenToWorld(_mousepos);
            ...
            if (!_isDissapearing)
            {
                var pos = Game.CameraMain.ScreenToWorld(_mousepos).ToPoint();
                pos.X = pos.X - 50;
                pos.Y = pos.Y - 50;
        ...
        {
            var pos = Game.CameraMain.ScreenToWorld(_mousepos);
            if (!Game.GameState.Party.RevealArea.InBounds(pos, out var newPos))
            {
                var newScreenPosition = Game.CameraMain.WorldToScreen(newPos);
