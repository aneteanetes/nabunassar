        ...
        public virtual FrameData GetFrameData(SpriterAnimation first, SpriterAnimation second, float targetTime, float deltaTime, float factor)
        {
            ...
            if (boneInfosA != null && boneInfosB != null)
            {
                ...
                for (int i = 0; i < boneInfosA.Length; ++i)
                {
                    ...
                    interpolated.Angle = SpriterDotNetMathHelper.CloserAngleLinear(boneA.Angle, boneB.Angle, factor);
            ...
            for (int i = 0; i < baseKey.ObjectRefs.Length; ++i)
            {
                ...
                info.Angle = SpriterDotNetMathHelper.CloserAngleLinear(interpolatedFirst.Angle, interpolatedSecond.Angle, factor);
                info.PivotX = SpriterDotNetMathHelper.Linear(interpolatedFirst.PivotX, interpolatedSecond.PivotX, factor);
                info.PivotY = SpriterDotNetMathHelper.Linear(interpolatedFirst.PivotY, interpolatedSecond.PivotY, factor);
