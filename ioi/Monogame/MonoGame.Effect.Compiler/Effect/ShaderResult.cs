...
using static MonoGame.Effect.Compiler.MonogameShaderCompilerEntry;
...
namespace MonoGame.Effect
{
    public class ShaderResult
    {
        ...
        static public ShaderResult FromFile(string path, MonogameEffectCompilerOptions options, IEffectCompilerOutput output = default)
        {
            if (output == default)
                output = new ConsoleEffectCompilerOutput();
            var effectSource = File.ReadAllText(path);
            return FromString(effectSource, path, options, output);
        }

        static public ShaderResult FromString(string effectSource, string filePath, MonogameEffectCompilerOptions options, IEffectCompilerOutput output)
        {
