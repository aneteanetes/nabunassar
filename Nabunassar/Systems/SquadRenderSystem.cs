...
using Nabunassar.Widgets.UserInterfaces.Combat.SquadView;
...
namespace Nabunassar.Systems
{
    internal class SquadRenderSystem : BaseSystem
    {
        ...
        public SquadRenderSystem(NabunassarGame game) : base(game, Aspect.All(typeof(SquadInBattle)))
        {
        }
        ...
        private void RenderSquad(SquadInBattle squadInMap, SpriteBatchKnowed sb)
        {
            ...
            foreach (var battler in battlers)
            {
                ...
                var slotScaled= Game.Camera.ScreenToWorld(slot);

                var pos = new Vector2((slot.X + (SlotSize.X / 2)), (slot.Y + (SlotSize.Y / 2)));
                pos = Game.Camera.ScreenToWorld(pos);
                ...
                if (Game.IsDrawBounds)
                {
                    ...
                    var slotSizeScaled = Game.Camera.ScreenToWorld(SlotSize);
                    sb.DrawRectangle(new RectangleF()
                    {
                        X = slotScaled.X,
                        Y = slotScaled.Y,
